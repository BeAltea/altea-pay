...
import { Eye, Sparkles, AlertCircle, Plus } from "lucide-react"
  ...
  try {
    ...
    const vmaxCustomers =
      allVmaxCustomers?.filter(
        (customer) =>
          customer.id_company?.toString().toLowerCase().trim() === profile.company_id.toString().toLowerCase().trim(),
      ) || []
    ...
    if (vmaxIds.length > 0) {
      ...
      if (creditProfiles.length === 0) {
        const cpfList = vmaxCustomers.map((c) => c["CPF/CNPJ"]?.replace(/\D/g, "")).filter(Boolean)
    ...
    const creditProfilesMap = new Map(creditProfiles.map((profile) => [profile.customer_id || profile.cpf, profile]))
          ...
          {clientesCompletos.map((cliente) => (
            <Card key={cliente.id} className="hover:shadow-lg transition-shadow">
              ...
              <CardContent className="space-y-3">
                {cliente.creditProfile?.score_assertiva !== null &&
                  cliente.creditProfile?.score_assertiva !== undefined && (
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {cliente.creditProfile.risk_level && (
                      <div className="flex items-center justify-between p-2 rounded bg-muted">
                        ...
                        <Badge
                          variant={
                            cliente.creditProfile.risk_level === "low"
                              ? "default"
                              : cliente.creditProfile.risk_level === "medium"
                                ? "secondary"
                                : "destructive"
                          }
                        >
                    ...
                    {cliente.creditProfile.sanctions_count !== null &&
                      cliente.creditProfile.sanctions_count !== undefined && (
                        <div className="flex items-center justify-between p-2 rounded bg-muted">
                          <span className="text-muted-foreground">Sanções:</span>
                          <Badge variant="destructive">{cliente.creditProfile.sanctions_count}</Badge>
                        </div>
                      )}
                    {cliente.creditProfile.has_sanctions !== null &&
                      cliente.creditProfile.has_sanctions !== undefined && (
                        <div className="flex items-center justify-between p-2 rounded bg-muted">
                    ...
                    {cliente.creditProfile.has_public_bonds !== null &&
                      cliente.creditProfile.has_public_bonds !== undefined && (
                        <div className="flex items-center justify-between p-2 rounded bg-muted">
                ...
                {cliente.creditProfile?.data_assertiva?.debitos &&
                  cliente.creditProfile.data_assertiva.debitos.length > 0 && (
