...
async function processSingleNegotiation(
  ...
  try {
    ...
    try {
      ...
      // Update agreement with ASAAS payment info - THIS IS THE CRITICAL UPDATE
      await supabase
        .from("agreements")
        .update({
...
async function sendNotificationsInBackground(
  ...
  try {
    ...
    if (params.notificationChannels.includes("whatsapp") && customerPhone && asaasCustomerId) {
      try {
        const { resendAsaasPaymentNotification, getAsaasCustomerNotifications, updateAsaasNotification } = await import("@/lib/asaas")
        const allNotifs = await getAsaasCustomerNotifications(asaasCustomerId)
        const paymentCreatedNotif = allNotifs.find((n: any) => n.event === "PAYMENT_CREATED")
        if (paymentCreatedNotif) {
          await updateAsaasNotification(paymentCreatedNotif.id, {
...
export async function sendBulkNegotiations(params: SendBulkNegotiationsParams) {
  try {
    ...
    if (!user) {
      return { success: false, error: "Não autenticado", sent: 0, results: [] }
    }
    ...
    if (profile?.role !== "super_admin") {
      return { success: false, error: "Sem permissão", sent: 0, results: [] }
    }
