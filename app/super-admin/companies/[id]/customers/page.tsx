...
import { Users, ArrowLeft, Mail, Phone, DollarSign, AlertTriangle, TrendingUp } from "lucide-react"
...
export default async function ManageCustomersPage({ params }: { params: { id: string } }) {
  ...
  const overdueCustomers = customersWithStats.filter((c) => c.status === "overdue").length
  const totalDebtAmount = customersWithStats.reduce((sum, c) => sum + c.totalDebt, 0)
  const totalOverdueAmount = customersWithStats.reduce((sum, c) => sum + c.overdueDebt, 0)
  const paidCustomers = customersWithStats.filter((c) => c.status === "paid").length
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-xs text-gray-500 mt-1">
                        <span>CPF: {customer.document}</span>
                        {customer.lastPayment && (
                          <>
                            <span>•</span>
                            <span>Último contato: {new Date(customer.lastPayment).toLocaleDateString("pt-BR")}</span>
                          </>
                        )}
                      </div>
                    </div>
