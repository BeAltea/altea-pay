-- Migration: Separate Localize Filters
-- Remove "Incompleto" filters and add specific "Sem Email (Ja Consultado)" and "Sem Telefone (Ja Consultado)"
--
-- BEFORE:
--   - no_email_never_queried (Sem Email, Nunca Consultado)
--   - no_phone_never_queried (Sem Tel., Nunca Consultado)
--   - incomplete_never_queried (Incompleto, Nunca Consultado) <-- REMOVED
--   - incomplete_already_queried (Incompleto, Ja Consultado) <-- REMOVED
--
-- AFTER:
--   - no_email_never_queried (Sem Email, Nunca Consultado)
--   - no_phone_never_queried (Sem Tel., Nunca Consultado)
--   - no_email_queried (Sem Email, Ja Consultado) <-- NEW
--   - no_phone_queried (Sem Tel., Ja Consultado) <-- NEW
...
BEGIN
  SELECT json_build_object(
    ...
    -- Sem Telefone, Ja Consultado (NEW)
    'no_phone_queried', (
      SELECT COUNT(*) FROM "VMAX" c
      WHERE c.id_company = p_company_id
        AND (c."Telefone 1" IS NULL OR TRIM(c."Telefone 1") = '')
